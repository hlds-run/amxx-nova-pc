...
#define GETENTRYNAME(hdr, entry)                                                                                       \
    (USENAMETABLE(hdr) ? (char*)((unsigned char*)(hdr) + (unsigned)((AMX_FUNCSTUBNT*)(entry))->nameofs)                \
...
{
    uint16_t val = 0x00ff;
    const auto ptr = (unsigned char*)&val;
...
{
    AMX_HEADER* hdr;
    #if (defined _Windows || defined LINUX || defined __FreeBSD__ || defined __OpenBSD__ || defined __APPLE__) &&      \
        !defined AMX_NODYNALOAD
...
int AMXAPI amx_NameLength(const AMX* amx, int* length)
{
    const auto hdr = (AMX_HEADER*)amx->base;
    assert(hdr != NULL);
    assert(hdr->magic == AMX_MAGIC);
    if (USENAMETABLE(hdr)) {
        const auto namelength = (uint16_t*)(amx->base + static_cast<unsigned>(hdr->nametable));
        *length = *namelength;
...
int AMXAPI amx_NumNatives(const AMX* amx, int* number)
{
    const auto hdr = (AMX_HEADER*)amx->base;
    assert(hdr != NULL);
    assert(hdr->magic == AMX_MAGIC);
...
int AMXAPI amx_NumPublics(const AMX* amx, int* number)
{
    const auto hdr = (AMX_HEADER*)amx->base;
    assert(hdr != NULL);
    assert(hdr->magic == AMX_MAGIC);
...
int AMXAPI amx_NumPubVars(const AMX* amx, int* number)
{
    const auto hdr = (AMX_HEADER*)amx->base;
    assert(hdr != NULL);
    assert(hdr->magic == AMX_MAGIC);
...
int AMXAPI amx_NumTags(const AMX* amx, int* number)
{
    const auto hdr = (AMX_HEADER*)amx->base;
    assert(hdr != NULL);
    assert(hdr->magic == AMX_MAGIC);
...
int AMXAPI amx_GetUserData(const AMX* amx, const long tag, void** ptr)
{
    int index;
...
int AMXAPI amx_SetUserData(AMX* amx, const long tag, void* ptr)
{
    int index;
...
AMX_NATIVE_INFO* AMXAPI amx_NativeInfo(const char* name, const AMX_NATIVE func)
{
...
int AMXAPI amx_PushArray(AMX* amx, cell* amx_addr, cell** phys_addr, const cell array[], const int numcells)
{
    cell* paddr;
...
int AMXAPI amx_PushString(
    AMX* amx, cell* amx_addr, cell** phys_addr, const char* string, const int pack, const int use_wchar)
{
    cell* paddr;
    int numcells, err;
...
{
    static const void* const amx_opcodelist[] = {&&op_none, &&op_load_pri, &&op_load_alt, &&op_load_s_pri,
        &&op_load_s_alt, &&op_lref_pri, &&op_lref_alt, &&op_lref_s_pri, &&op_lref_s_alt, &&op_load_i, &&op_lodb_i,
        &&op_const_pri, &&op_const_alt, &&op_addr_pri, &&op_addr_alt, &&op_stor_pri, &&op_stor_alt, &&op_stor_s_pri,
        &&op_stor_s_alt, &&op_sref_pri, &&op_sref_alt, &&op_sref_s_pri, &&op_sref_s_alt, &&op_stor_i, &&op_strb_i,
        &&op_lidx, &&op_lidx_b, &&op_idxaddr, &&op_idxaddr_b, &&op_align_pri, &&op_align_alt, &&op_lctrl, &&op_sctrl,
        &&op_move_pri, &&op_move_alt, &&op_xchg, &&op_push_pri, &&op_push_alt, &&op_push_r, &&op_push_c, &&op_push,
        &&op_push_s, &&op_pop_pri, &&op_pop_alt, &&op_stack, &&op_heap, &&op_proc, &&op_ret, &&op_retn, &&op_call,
        &&op_call_pri, &&op_jump, &&op_jrel, &&op_jzer, &&op_jnz, &&op_jeq, &&op_jneq, &&op_jless, &&op_jleq,
        &&op_jgrtr, &&op_jgeq, &&op_jsless, &&op_jsleq, &&op_jsgrtr, &&op_jsgeq, &&op_shl, &&op_shr, &&op_sshr,
        &&op_shl_c_pri, &&op_shl_c_alt, &&op_shr_c_pri, &&op_shr_c_alt, &&op_smul, &&op_sdiv, &&op_sdiv_alt, &&op_umul,
        &&op_udiv, &&op_udiv_alt, &&op_add, &&op_sub, &&op_sub_alt, &&op_and, &&op_or, &&op_xor, &&op_not, &&op_neg,
        &&op_invert, &&op_add_c, &&op_smul_c, &&op_zero_pri, &&op_zero_alt, &&op_zero, &&op_zero_s, &&op_sign_pri,
        &&op_sign_alt, &&op_eq, &&op_neq, &&op_less, &&op_leq, &&op_grtr, &&op_geq, &&op_sless, &&op_sleq, &&op_sgrtr,
        &&op_sgeq, &&op_eq_c_pri, &&op_eq_c_alt, &&op_inc_pri, &&op_inc_alt, &&op_inc, &&op_inc_s, &&op_inc_i,
        &&op_dec_pri, &&op_dec_alt, &&op_dec, &&op_dec_s, &&op_dec_i, &&op_movs, &&op_cmps, &&op_fill, &&op_halt,
        &&op_bounds, &&op_sysreq_pri, &&op_sysreq_c, &&op_file, &&op_line, &&op_symbol, &&op_srange, &&op_jump_pri,
        &&op_switch, &&op_casetbl, &&op_swap_pri, &&op_swap_alt, &&op_pushaddr, &&op_nop, &&op_sysreq_d, &&op_symtag,
        &&op_break};
    AMX_HEADER* hdr;
    AMX_FUNCSTUB* func;
    ...
    for (;;) {
        ...
        switch (op) {
            ...
            case OP_MOVS:
                ...
                if (pri + offs > hea && pri + offs < stk ||
                    static_cast<ucell>(pri + offs) > static_cast<ucell>(amx->stp)) {
                    ABORT(amx, AMX_ERR_MEMACCESS)
                }
                if (alt >= hea && alt < stk || static_cast<ucell>(alt) >= static_cast<ucell>(amx->stp)) {
                    ABORT(amx, AMX_ERR_MEMACCESS)
...
int AMXAPI amx_SetDebugHook(AMX* amx, const AMX_DEBUG debug)
{
    assert(amx != NULL);
    amx->debug = debug;
    return AMX_ERR_NONE;
...
int AMXAPI amx_RaiseError(AMX* amx, const int error)
{
    assert(error > 0);
    amx->error = error;
    return AMX_ERR_NONE;
...
int AMXAPI amx_Release(AMX* amx, const cell amx_addr)
{
    if (amx->hea > amx_addr) {
        amx->hea = amx_addr;
    }
...
int AMXAPI amx_SetString(cell* dest, const char* source, const int pack, const int use_wchar, const size_t size)
...
int AMXAPI amx_GetString(char* dest, const cell* source, const int use_wchar, const size_t size)
{
    int len = 0;
    #if defined AMX_ANSIONLY
    (void)use_wchar;
