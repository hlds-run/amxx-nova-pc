...
namespace Amxx {
    ...
    void Writer::open()
    {
        ...
        if (!is_open()) {
            throw Binary::Exceptions::FileOpenError{file_path().string(), "writing"};
        }
    }
    ...
    void Writer::write_header(const Header& header, const std::streamoff offset, const std::ios::seekdir dir) const
    {
        if (!bin_writer_.seek(offset, dir)) {
            throw Binary::Exceptions::SeekError{offset, dir, "AMXX file header"};
        }
    ...
    {
        if (!bin_writer_.seek(offset, dir)) {
            throw Binary::Exceptions::SeekError{offset, dir, "AMXX file entry"};
        }
    ...
    void Writer::throw_stream_exception([[maybe_unused]] Binary::Writer::Error error, const std::string& context)
    {
