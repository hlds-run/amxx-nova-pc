                ...
                if (lex(&val, &str) == tSYMBOL) {
                    if (strcmp(str, "amxlimit") == 0) {
                        ...
                        lptr = (unsigned char*)strchr(
                            (char*)lptr, '\0'); /* skip to end (ignore "extra characters on line") */
                    }
                    else if (strcmp(str, "dynamic") == 0) {
                        preproc_expr(&sc_stksize, NULL);
                    }
                    else if (!strcmp(str, "library") || !strcmp(str, "reqlib") || !strcmp(str, "reqclass") ||
                             !strcmp(str, "loadlib") || !strcmp(str, "explib") || !strcmp(str, "expclass") ||
            ...
            if (*lptr != '\0') {
                ...
                switch (tok) {
                    ...
                    case tSYMBOL:
                        ...
                        if (sym == NULL ||
                            (sym->ident != iFUNCTN && sym->ident != iREFFUNC && (sym->usage & uDEFINE) == 0)) {
...
char* sc_tokens[] = {"*=", "/=", "%=", "+=", "-=", "<<=", ">>>=", ">>=", "&=", "^=", "|=", "||", "&&",
    ...
    "const", "continue", "default", "defined", "do", "else", "enum", "exit", "for", "forward", "goto", "if", "native",
    "new", "operator", "public", "return", "sizeof", "sleep", "state", "static", "stock", "switch", "tagof", "while",
    "#assert", "#define", "#else", "#elseif", "#emit", "#endif", "#endinput", "#endscript", "#error", "#file", "#if",
    "#include", "#line", "#pragma", "#tryinclude", "#undef", ";", ";", "-integer value-", "-rational value-",
        ...
        *lexvalue = _lexval = litidx;
        ...
        for (;;) {
            ...
            *cat = '\0'; /* terminate string */
            ...
            if (!freading || !(*lptr == '\"' || (*lptr == sc_ctrlchar && *(lptr + 1) == '\"') ||
                                 (*lptr == '!' && *(lptr + 1) == '\"') ||
